...
# IMPORT ------------------------------------------------------------------
  ...
  df_dp <- tame_dp("Data/PSNUxIM_Tanzania_June 05 2023_FINAL.xlsx", 
                   type = "PSNUxIM")
...
#FY 2023 Target Summary
  df_summary <- df_dp %>% 
    ...
    mutate(indicator = factor(indicator, tbl_ind),
           ...
           snuprioritization = ifelse(psnu == "_Military Tanzania", "mil", "Scale Up")) %>% 
  ...
  saveWorkbook(wb, file.path(folderpath_output, "COP23_TZA_Table-Check.xlsx"), overwrite = TRUE)
