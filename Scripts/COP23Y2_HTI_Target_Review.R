...
# VIZ ---------------------------------------------------------------------
  ...
  df_viz <- df_agg %>% 
    ...
    mutate(fill_color = case_when(funding_agency == "USAID" & delta >= 0 ~ hw_electric_indigo,
           ...
           indicator_val = case_when(indicator == "HTS_RECENT" ~ glue("{indicator} [FY25 USAID Targets: {label_comma(1)(usaid_value)}]"),
...
# MUNGE SHARES ------------------------------------------------------------
  ...
  df_viz %>% 
    ...
    labs(x = NULL, y = NULL,
         title = glue("FY25 USAID TARGETS TEND TO SEE SIMILAR <span style='color: {hw_electric_indigo};'>GROWTH</span> and <span style='color: {hw_orchid_bloom};'>DECLINE</span> PATTERNS COMPARED TO <span style='color:{hw_slate};'>OTHER AGENCIES</span>"),
  ...
  df_agg %>% 
    ...
    labs(x = NULL, y = NULL,
         ...
         subtitle = "USAID has the major share of PEPFAR/Haiti's targets for PP_PREV, OVC, PrEP, and HTS_SELF",
