...
# Country: Zambia
# Target Setting Tool Last Loaded: 2024-03-21 10:53:54 AM
# Target Summary: Total Population and KP

# GLOBAL VARIABLES --------------------------------------------------------
  
  ref_id <- "c3e0e3b5"  #a reference to be places in viz captions 
  
  path_paw <- c("../Downloads/COP23_FY25 & ROP24_FY25-26 Target Setting Tool Dossier.csv",
                "../Downloads/COP23_FY25 & ROP24_FY25-26 Target Setting Tool Dossier_KP.csv")
...
# VIZ ---------------------------------------------------------------------
  ...
  df_viz <- df_agg %>% 
    ...
    mutate(fill_color = case_when(funding_agency == "USAID" & delta >= 0 ~ hw_electric_indigo,
           ...
           indicator_val = case_when(indicator == "PP_PREV" ~ glue("{indicator} [FY25 USAID Targets: {label_comma(1)(usaid_value)}]"),
...
# MUNGE SHARES ------------------------------------------------------------
  ...
  df_viz %>% 
    ...
    labs(x = NULL, y = NULL,
         title = glue("FY25 USAID TARGETS TEND SEE <span style='color: {hw_orchid_bloom};'>DECLINES</span> COMPARED TO FY23 TARGETS"),
         ...
         Source: COP23_FY25 & ROP24_FY25-26 Target Setting Tool Dossier [2023-03-26] | Ref id: {ref_id}")) +
    si_style_xgrid() +
    theme(plot.title = element_markdown(),
          plot.subtitle = element_markdown())

  si_preview()
  si_save("../Downloads/COP23Y2_ZAM_target-delta.png")
  ...
  df_agg %>% 
    ...
    labs(x = NULL, y = NULL,
         ...
         subtitle = "USAID has the major share of PEPFAR/Zambia's targets in only 3 indicators - OVC_HIVSTAT, OVC_SERV, and PP_PREV",
         caption = glue("Source: COP23_FY25 & ROP24_FY25-26 Target Setting Tool Dossier [2023-03-26] | Ref id: {ref_id}")) +
    si_style_nolines() +
  ...
  si_preview()
  si_save("../Downloads/COP23Y2_ZAM_target-share.png")
